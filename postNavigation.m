...
%% Find pseudoranges ======================================================
    ...
    [navSolutions.dopps(:, currMeasNr)]=  ...
                     calculateDopps(trackResults, currMeasSample, ...
                     activeChnList, settings); % add by Yixin on 2025/03/11
...
%% Find receiver position and velocity =================================================
    ...
    if size(activeChnList, 2) > 3
        ...
        navSolutions.satPositions{currMeasNr} = satPositions; % add by Yixin
        navSolutions.satVelocity{currMeasNr} = satVelocity;% add by Yixin
        ...
        % Calculate receiver position with least square estimation
        ...
        % Calculate receiver position with least square estimation
        receiver_velocity = ...
            leastsquareVel(satPositions, satVelocity, xyzdt, ...
            navSolutions.dopps(:, currMeasNr), activeChnList,settings); % add by Yixin
        
        % satPositions_fine=satPositions(:,[1 3 4]);
        % satVelocity_fine=satVelocity(:,[1 3 4]);
        % dopp_fine=navSolutions.dopps([1 3 4], currMeasNr);
        % activeChnList_fine=3;
        % receiver_velocity_fine = ...
        %     leastsquareVel(satPositions_fine, satVelocity_fine,xyzdt, ...
        %     dopp_fine, activeChnList_fine,settings); % add by Yixin
        
        % Calculate receiver position and velocity with EKF
        if currMeasNr == 1
            X_k = [xyzdt(1:3),0,0,0,xyzdt(4)]';
        else
            X_k(7)=0;  % rcv clk bias has been corrected after 1st epoch, so it should be 0 --sbs
        end
        % X_k=[0,0,0,0,0,0,0]';
        rcv_vel=[0,0,0]';
        % rcv_vel=receiver_velocity(1:3)';
        P_k = diag([100^2, 100^2, 100^2, 10^2, 10^2, 10^2, 10^3]); % covariance estimation

        [X_e, P_k]= gnss_ekf(X_k,P_k,rcv_vel,clkCorrRawP,satPositions,settings);
        ...
        navSolutions.Vx(currMeasNr)  = receiver_velocity(1);
        navSolutions.Vy(currMeasNr)  = receiver_velocity(2);
        navSolutions.Vz(currMeasNr)  = receiver_velocity(3);    

        % navSolutions.Vx_fine(currMeasNr)  = receiver_velocity_fine(1);
        % navSolutions.Vy_fine(currMeasNr)  = receiver_velocity_fine(2);
        % navSolutions.Vz_fine(currMeasNr)  = receiver_velocity_fine(3);    

        % Receiver position in ECEF estimated with EKF
        navSolutions.X_ekf(currMeasNr)  = X_e(1);
        navSolutions.Y_ekf(currMeasNr)  = X_e(2);
        navSolutions.Z_ekf(currMeasNr)  = X_e(3);       
        
        % Receiver velocity in ECEFestimated with EKF
        navSolutions.Vx_ekf(currMeasNr)  = X_e(4);
        navSolutions.Vy_ekf(currMeasNr)  = X_e(5);
        navSolutions.Vz_ekf(currMeasNr)  = X_e(6);   
...
%% Coordinate conversion ==================================================
        ...
        [navSolutions.E_ekf(currMeasNr), ...
         navSolutions.N_ekf(currMeasNr), ...
         navSolutions.U_ekf(currMeasNr)] = cart2utm(X_e(1), X_e(2), ...
                                                X_e(3), ...
                                                navSolutions.utmZone);
